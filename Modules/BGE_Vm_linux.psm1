function BGE_Vm_Linux {
    ...
    function Show-OSMenu {
        ...
        Write-Host "1: Press '1' Ubuntu 18.4"
        Write-Host "2: Press '2' kemur seinna"
        Write-Host "3: Press '3' kemur seinna"
    ...
    do {
        ...
        switch ($input) {
            '1' {
                $global:TemplateLocation = "$global:template\$global:Ubuntu18"
            } '2' {
                $global:TemplateLocation = "$global:template\$global:Centos7"
            } '3' {
                $global:TemplateLocation = "$global:template\$global:Debian"
                  } 'q' {
                Return 
            }
        }
        ...
        switch ($input) {
            'C' {
                If ($DHCP -eq 'true') {
                    ...
                    $DNSDomain = "1"
                    $DHCP = "true"
                    BGE_create_vm_L $Name $CpuCount $RAM $net $IpAddress $DefaultGW $DNSServer $DNSDomain $DHCP
                }
                Else {
                    BGE_create_vm_L $name $CpuCount $RAM $net $IpAddress $DefaultGW $DNSServer $DNSDomain $DHCP
                }
